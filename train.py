...
def set_default():
    ...
    if config.enable_modelarts:
        ...
        print('in default:',
              config.data_root)  # s3://open-data/attachment/7/e/7ebc4aee-1187-4c6c-89b5-67c438cf7eae7ebc4aee-1187-4c6c-89b5-67c438cf7eae/coco2017/train2017
        config.annFile = os.path.join(config.data_url, 'coco2017/annotations')
        print('in default:',
              config.annFile)  # s3://open-data/attachment/7/e/7ebc4aee-1187-4c6c-89b5-67c438cf7eae7ebc4aee-1187-4c6c-89b5-67c438cf7eae/coco2017/annotations
        outputs_dir = os.path.join(config.outputs_dir, config.ckpt_path)
        print('in default:', outputs_dir)  # /home/work/user-job-dir/outputs/model/./
        ...
        outputs_dir = config.ckpt_path #important
...
def modelarts_pre_process():
    ...
    if config.need_modelarts_dataset_unzip:
        ...
        print("save_dir_1:", save_dir_1)  # /home/work/user-job-dir/inputs/data/
        print('zip_file:', zip_file_1)  # /home/work/user-job-dir/inputs/data/coco2017.zip
...
def parser_init():
    ...
    parser.add_argument('--min_lr_ratio', required=False, default=0.01)
...
def run_train():
    ...
    os.system("ls /cache/dataset/coco2017")
    ...
    if config.enable_modelarts:
        ...
        # s3://open-data/attachment/7/e/7ebc4aee-1187-4c6c-89b5-67c438cf7eae7ebc4aee-1187-4c6c-89b5-67c438cf7eae/coco2017/train2017  -->  /home/work/user-job-dir/inputs/data/0
        print('copy from {} to {}'.format(config.data_root, local_data_url))
        mox.file.copy_parallel(config.data_root, local_data_url)
        config.data_dir = os.path.join(config.data_path, 'coco2017')  # /home/work/user-job-dir/inputs/data/0
        print("config.data_root:", config.data_root)
        print("config.anno_file:", config.annFile)
        print('locol_annFile:', local_annFile)
        mox.file.copy_parallel(config.annFile, local_annFile)
        config.annFile = os.path.join(local_data_url, 'instances_train2017.json')
        print('config.annFile:', config.annFile)
        # config.data_dir = local_data_url
    ...
    if config.backbone == "yolox_darknet53":
        ...
        backbone = "yolopafpn"
    test_base_network = DetectionBaseBlock(config, backbone=backbone)
    train_base_network = DetectionBaseBlock(config, backbone=backbone)
    test_network = DetectionTestBlock(test_base_network)
    network = DetectionTrainBlock(train_base_network)  # train network
    ...
    config.data_root = os.path.join(config.data_dir, 'train2017')
    print('2:config.data_root', config.data_root)
    config.annFile = os.path.join(config.data_dir, 'annotations/instances_train2017.json')
    print('2 config.annFile', config.annFile)
    print('before ds:', config.data_root)
    print('before ds:', config.annFile)
    ...
    network_ema = TrainOneStepWithEMA(network, opt, update_cell,
                                      ema=config.ema, decay=0.9999, updates=0).set_train()
